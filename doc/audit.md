...
# Внутренний аудит-раунд: верификация внешнего v15 (2026-09-11, коммит 948fd4a)

Проверены исполнимые утверждения v15, всё — чтением кода и локальными прогонами без изменения репо.

- **Потеря штампов fallback (§5 v15) — ПОДТВЕРЖДЕНА репродукцией.** Книга без `document-info`, legacy-метка в теге, второй прогон с чисто lossless-изменением байтов: выход без `fb2opt-lossy`, `marked=0`. Механика в коде: цикл `optimize_fb2_payload` снимает старую метку через `_strip_mark_from_attrs`, а fallback-ветвь `_write_lossy_token` штампует только `mark_set` (куда попадают лишь fresh lossy-победы). Прямой ущерб мал (перекодировки нет, «already optimal»), но защита снята молча.
- **`content-type` во fallback (§6) — ПОДТВЕРЖДЕНА чтением кода.** `skeleton_late` правит `content-type` (jpg→png после бинаризации), затем fallback-ветвь перезаписывает весь тег из stale `real_attrs` — правка затирается. Только путь без `document-info` (схемно-невалидные книги), приоритет низкий.
- **Дыра глобального SSIM (§3) — механизм верен, лекарство проверено локально.** Плиточная статистика «доля около-пороговых пикселей 8×8» на моих страницах: чистый текст worst-tile 0.0, текст+фото 0.45, фото 1.0 — порог 0.05 из v15 отделяет с запасом. Дешёвый PIL-предфильтр до ffmpeg считает секунды.
- **Порог 0.85 для сканов (§4) — правдоподобно, стенда нет.** Принято как показание; вывод (README + подсказка) не требует проверки байтами.
- Остальное (§6 lint/C901, §2 замеры корпуса) — показания аудитора, претензий к методу нет; lossless-корпус у меня не гонялся (стенда нет), 150 тестов зелёные во всех трёх симуляциях окружения.

**Вердикт: [REJECTED]** — тихая порча фото-вклеек под SSIM-гейтом (SSIM 0.9285 при 0.158 в области фото) нарушает декларированный контракт `--lossy` («guarded by a metric»), плюс детерминированная потеря штампов. Фиксы точечные:
1. Плиточный предфильтр «точно скан» перед `_lossy_bilevel` (порог 0.05, verified выше).
2. Fallback-цикл: штамповать все картинки с бывшим штампом, не только `mark_set` (одна строка по v15).
3. Fallback: `content-type` из обновлённых атрибутов, не из `real_attrs`.
4. README + `--lossy`-подсказка: для сканов `--lossy-ssim 0.85`; повтор на полном разрешении выигрыша не гарантирует.
За рамками (владельцы): CI-job с реальными ffmpeg+ect (класс «байты стали больше/хуже» hermetic-тесты не ловят), `doc/roadmap.md` всё ещё нет.

*Аудитор: Senior Code Forensics / AppSec QA. Кодер: реализацию маркеров/тегов не начинать до закрытия 1–4? — на усмотрение владельца: 1–3 чинят активную порчу данных, маркеры — новая фича.*
